...
func (c *FabricClient) Invoke(ctx context.Context, identity Identity, chainCode ChainCode, peers []string, orderer string) (*InvokeResponse, error) {
	...
	endorsements := sendToPeers(ctx, execPeers, proposal)
	transaction, err := createTransaction(prop.proposal, endorsements)
	if err != nil {
		return nil, err
	}
	signedTransaction, err := c.Crypto.Sign(transaction, identity.PrivateKey)
...
func (c *FabricClient) QueryTransaction(ctx context.Context, identity Identity, channelId string, txId string, peers []string) ([]*QueryTransactionResponse, error) {
	...
	for idx, p := range r {
		qtr := QueryTransactionResponse{Error: p.Err}
		if p.Err != nil {
			qtr.Error = p.Err
		} else {
			tx, err := decodeTransaction(p.Response.Response.GetPayload())
			if err != nil {
				qtr.Error = err
			}
			qtr.Transaction = tx
...
func (c *FabricClient) QueryBlockByTxID(ctx context.Context, identity Identity, channelId string, txId string, peers []string) ([]*common.Block, error) {
	execPeers := c.getPeers(peers)
	if len(peers) != len(execPeers) {
		return nil, ErrPeerNameNotFound
	}
...
func (c *FabricClient) QueryBlockByHash(ctx context.Context, identity Identity, channelId string, hash []byte, peers []string) ([]*common.Block, error) {
	execPeers := c.getPeers(peers)
	if len(peers) != len(execPeers) {
		return nil, ErrPeerNameNotFound
	}
	chainCode := ChainCode{
		...
		Name:      QSCC,
		Type:      ChaincodeSpec_GOLANG,
		Args:      []string{"GetBlockByHash", channelId},
		ArgBytes:  hash,
	}
...
func (c *FabricClient) QueryBlockByNumber(ctx context.Context, identity Identity, channelId string, number uint64, peers []string) ([]*common.Block, error) {
	execPeers := c.getPeers(peers)
	if len(peers) != len(execPeers) {
		return nil, ErrPeerNameNotFound
	}
