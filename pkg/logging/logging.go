...
type Config struct {
	...
	// enc.ParseFormat for details on the supported verbs.
	//
...
func (s *Logging) Apply(c Config) error {
	...
	if c.LogSpec == "" {
		c.LogSpec = os.Getenv("GLIBS_LOGGING_SPEC")
	}
	if c.LogSpec == "" {
...
func (s *Logging) ZapLogger(name string) *zap.Logger {
	...
	core := &Core{
		...
		Encoders: map[Encoding]zapcore.Encoder{
			...
			CONSOLE: enc.NewFormatEncoder(s.multiFormatter),
		},
		Selector: s,
		Output:   s,
	}
...
// used to determine which log levels are enabled.
func (s *Logging) Logger(name string) *GlibsLogger {
	zl := s.ZapLogger(name)
	return NewGlibsLogger(zl)
}
