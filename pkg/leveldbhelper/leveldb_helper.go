...
func (dbInst *DB) Open() error {
	...
	var err error
	var dirEmpty bool
	if dirEmpty, err = CreateDirIfMissing(dbPath); err != nil {
		return errors.WithMessage(err, "Error creating dir if missing")
	}
	dbOpts.ErrorIfMissing = !dirEmpty
	if dbInst.db, err = leveldb.OpenFile(dbPath, dbOpts); err != nil {
		return errors.WithMessage(err, "Error opening leveldb")
	}
...
//	var err error
//	var dirEmpty bool
//	if dirEmpty, err = CreateDirIfMissing(f.filePath); err != nil {
