...
func PackZip(path string, zipName string) error {
	z := archiver.Zip{
	...
	return z.Archive([]string{path}, zipName)
}

func getSubDir(dirPath string) (string, error) {
	...
	if dirCount == 1 {
		return filepath.Join(dirPath, dirName), nil
	}
...
func UnpackToDir(r io.Reader, destPath string, dir string) error {
	...
	tempDir, err := ioutil.TempDir(destPath, "tmp")
	if err != nil {
		return errors.New("create temp dir error")
	}

	defer os.RemoveAll(tempDir)

	_, err = Unpack(r, tempDir)
	if err != nil {
		return err
	}

	subDir, err := getSubDir(tempDir)
	if err != nil {
		return err
	}

	return os.Rename(subDir, filepath.Join(destPath, dir))
}

func RenameHash(path string, ren func(name string) string) error {
	f, err := os.Open(path)
	if err != nil {
		return err
	}
